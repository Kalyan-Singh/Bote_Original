import React, { useEffect, useState } from "react";
import { Bote } from "./Contracts";
import { abi_Bote } from "./Contracts";
import {
  useContractRead,
  useAccount,
  usePrepareContractWrite,
  useContractWrite,
} from "wagmi";
import { BigNumber } from "ethers";

interface Poll {
  id: BigNumber;
  state: string;
  creator: string;
  duration: Number;
  parties: readonly string[];
  votes: readonly Number[];
}

function MyPolls() {
  const [polls, setpolls] = useState<Poll[] | undefined>();
  const [pollid, setpollid] = useState<BigNumber | undefined>();
  const { config, isError } = usePrepareContractWrite({
    address: Bote,
    abi: abi_Bote,
    functionName: "startPoll",
    args: [pollid!],
  });
  const { writeAsync, isLoading } = useContractWrite(config);
  const { address } = useAccount();
  const { data } = useContractRead({
    address: Bote,
    abi: abi_Bote,
    functionName: "myPolls",
    overrides: { from: address },
    onSuccess: async (data) => {
      const Polls: Poll[] = data.map((poll) => {
        let ps: string;
        if (poll.c_state == 0) {
          ps = "started";
        } else if (poll.c_state == 1) {
          ps = "ended";
        } else {
          ps = "notStarted";
        }

        const npoll: Poll = {
          id: poll.PollID,
          state: ps,
          creator: poll.creator,
          duration: poll.duration.toNumber(),
          parties: poll.parties,
          votes: poll.votes.map((vote) => {
            return vote.toNumber();
          }),
        };
        return npoll;
      });
      setpolls(Polls);
    },
  });

  useEffect(() => {
    console.log(pollid);
    writeAsync?.();
  }, [pollid]);
  if (!address) {
    return (
      <div className="justify-center flex mt-44 text-blue-700">
        Connect A Wallet
      </div>
    );
  }
  if (!polls) {
    return (
      <div role="status" className="flex justify-center mt-52">
        <svg
          aria-hidden="true"
          className="mr-2 w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
          viewBox="0 0 100 101"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
            fill="currentColor"
          ></path>
          <path
            d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
            fill="currentFill"
          ></path>
        </svg>
        <span className="sr-only">Loading...</span>
      </div>
    );
  }
  if (isLoading) {
    return (
      <div role="status" className="flex justify-center mt-52">
        <svg
          aria-hidden="true"
          className="mr-2 w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600"
          viewBox="0 0 100 101"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
            fill="currentColor"
          ></path>
          <path
            d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
            fill="currentFill"
          ></path>
        </svg>
        <span className="sr-only">Loading...</span>
      </div>
    );
  }

  if (polls) {
    return (
      <div className="flex mt-2">
        {polls.map((poll) => {
          return (
            <div className="p-6 bg-white border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700 ml-2 max-w-md">
              <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{`Poll Id ${poll.id.toNumber()}`}</h5>
              <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">
                {`Parties ${poll.parties.map((party) => {
                  return party;
                })} `}
              </p>
              <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">
                {`Duration ${poll.duration} Minutes`}
              </p>
              {poll.state == "notStarted" ? (
                <button
                  className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                  onClick={() => {
                    setpollid(poll.id);
                  }}
                >
                  Start Poll
                  <svg
                    aria-hidden="true"
                    className="w-4 h-4 ml-2 -mr-1"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      fill-rule="evenodd"
                      d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
                      clip-rule="evenodd"
                    ></path>
                  </svg>
                </button>
              ) : (
                <></>
              )}
            </div>
          );
        })}
      </div>
    );
  }
}

export default MyPolls;
